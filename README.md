...
AstrBot 系统级网络流量监控插件，支持实时速率与本月已记录流量统计。
...
- **本月已记录流量持久化**：每轮成功采样的上下行增量会写入 AstrBot 插件数据目录，并按自然月自动归零。
- **自动回退**：启动时逐个数据源试读；运行期 active provider 连续失败 3 次后尝试切换备用 provider。

> 本月已记录流量从插件开始运行并成功采样后计算；插件未运行期间已经产生的系统流量无法反推。
...
| 指令 | 说明 |
|------|------|
| `/net` | 实时速率（滑动窗口平均+瞬时）+ 本月已记录流量 + 本次启动累计 + 运行时长 + 数据源/网卡数 |
...
monthly_usage.py       本月已记录流量 JSON 持久化 + 自然月轮转
...
- **当前获取的系统总量是什么**：底层 `/proc/net/dev` / `psutil.net_io_counters(pernic=True)` 返回的是系统启动以来的网卡累计计数。插件不会把这个值直接当作自然月完整流量，而是只把相邻两次采样的正向差值累加到本月已记录流量里。
